class Scene2 extends Phaser.Scene {
    ...
    create() {
        ...
        for (var i = 0; i < 10; i++) {
            for (var j = 0; j < 10; j++) {
                ...
                var graphic = this.add.graphics(this)
                    .fillStyle(color, 1)
                    .fillRect(x, y, squareSize, squareSize)
            }
        }

        this.player1 = this.add.sprite(squareSize / 2, config.height - squareSize / 2, "player1");
    ...
    movePlayer(targetCell) {
        ...
        if (gameSettings.currentCell.x < 9 && gameSettings.currentCell.y % 2 !== 0) {
            gameSettings.currentCell.x += 1;
        } else if (gameSettings.currentCell.x > 0 && gameSettings.currentCell.y % 2 === 0) {
            gameSettings.currentCell.x -= 1;
        } else if (gameSettings.currentCell.x === 9 && gameSettings.currentCell.y % 2 !== 0) {
